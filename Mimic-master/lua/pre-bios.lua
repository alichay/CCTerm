...
xpcall = function(_fn, _fnErrorHandler)
	...
	debugLib.sethook(co, function()
		if os.clock() >= coroutineClock + 3.5 then
			console.log("Lua: Too long without yielding 30 pb old")
			error("Too long without yielding 31 pb old", 2)
		end
	end, "", 10000)
	...
	while coroutine.status(co) ~= "dead" do
		...
		debugLib.sethook(co, function()
			if os.clock() >= coroutineClock + 3.5 then
				console.log("Lua: Too long without yielding 44 pb old")
				error("Too long without yielding 45 pb old", 2)
			end
		end, "", 10000)
